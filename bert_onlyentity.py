...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.position_embeddings = nn.Embedding(100, 200)
        ...
        self.coref_weights = nn.Linear(6*config.hidden_size+200*3+200, config.hidden_size)
        self.coref_classifier = nn.Linear(config.hidden_size, 1)
    ...
    def forward(
        ...
        #batch_coref_position_embeddings = self.coref_position_embeddings(coref_position_2d).to(combined_tensors.device)
        ...
        if labels is not None and rel_labels is not None:
            ...
            if attention_mask is not None:
                ...
                #loss2 = loss_fct(coref_logits.view(-1, 1), coref_labels.view(-1))
